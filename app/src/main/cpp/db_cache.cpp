...
bool AnalysisDb::store_functions(int64_t binary_id,
    ...
    exec("BEGIN");
    sqlite3_stmt* stmt = nullptr;
    const char* sql = "INSERT INTO functions(binary_id,address,size,name,end_address) VALUES(?,?,?,?,?)";
    sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr);
...
std::vector<DbFunction> AnalysisDb::get_functions(int64_t binary_id) {
    ...
    sqlite3_stmt* stmt = nullptr;
    const char* sql = "SELECT id,address,size,name,end_address FROM functions WHERE binary_id=? ORDER BY address";
    sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr);
    sqlite3_bind_int64(stmt, 1, binary_id);
