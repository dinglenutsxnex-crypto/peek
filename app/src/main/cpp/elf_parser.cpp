...
ElfParseResult elf_parse(const std::string& path) {
    ...
    auto parse_syms = [&](uint64_t sym_off, uint64_t sym_size,
        ...
        for (size_t si = 0; si < count; ++si) {
            ...
            if (ps.type == STT_FUNC && ps.address != 0 && ps.size > 0 && !ps.is_import) {
                ParsedFunction pf;
    ...
    LOGI("Parsed ELF: %zu sections, %zu symbols, %zu functions",
         res.sections.size(), res.symbols.size(), res.functions.size());
